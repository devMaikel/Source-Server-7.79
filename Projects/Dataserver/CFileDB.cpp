...
void CFileDB::DBGetSelChar(st_CharList *p, STRUCT_ACCOUNT *file)
{
	for (INT32 i = 0; i < 4; i++)
	{
		...
		memcpy(p->Equip[i], file->Mob[i].Player.Equip, sizeof STRUCT_ITEM * 20);
...
INT32 CFileDB::ProcessMessage(char *msg, INT32 conn)
{
	...
	{//00412EE7
		STRUCT_SAVECHARACTER*p = reinterpret_cast<STRUCT_SAVECHARACTER*>(Header);
		...
		memcpy(Account[idx].Account.Storage.Item, p->Storage, sizeof STRUCT_ITEM * 168);
	...
	case _MSG_DBSaveMob:
	{
		STRUCT_SAVECHARACTER*p = (STRUCT_SAVECHARACTER*)(Header);
		INT32 index = GetIndex(conn, Header->ClientId);
		...
		{
			// Log(err, savemob1 "
			printf("_MSG_DBSaveMob %s - err, savemob1. Slot %d\n", Account[index].Account.Username, slot);
			return 1;
		}
		...
		{
			// Log(err, savemob2 "
			printf("_MSG_DBSaveMob %s - %d err, savemob2\n", Account[index].Account.Username, p->CharSlot);
			return true;
		}
		...
		{
			// Log(err, savemob3 
			printf("_MSG_DBSaveMob %s - err, savemob3\n", Account[index].Account.Username);
		...
		memcpy(Account[index].Account.Storage.Item, p->Storage, sizeof STRUCT_ITEM * 168);
	...
	case _MSG_DBAccountLogin:
	{
		...
		{
			FILE* pFile = NULL;
			fopen_s(&pFile, "..\\DATA\\ImportUser\\newacc.txt", "w+");
			fprintf(pFile, "%s %s 0", p->Login, p->Password);
			fclose(pFile);
		...
		memcpy(&selChar.Storage[0], &account.Storage.Item[0], sizeof STRUCT_ITEM * 168);
	...
	case PutInSealPacket:
		...
		info.Mob.DailyQuest.BattlePass = STRUCT_ITEMDATE{};
