...
void ProcessSecTimer()
{
	...
	cFileDB.DBWriteAccount(&account);
...
void ImportPass()
{
	...
	char path[256] = { 0 };
	char arq[256] = { 0 };
	strncpy_s(path, "..\\DATA\\ImportPass\\*.txt", 256);
	...
	do
	{
		...
		sprintf_s(arq, "..\\DATA\\ImportPass\\%s", file.name);
		FILE* pFile;
...
void ImportUser()
{
	...
	char path[256];
	char arq[256];
	strncpy_s(path, "..\\DATA\\ImportUser\\*.txt", 20);
	...
	do
	{
		...
		sprintf_s(arq, "..\\DATA\\ImportUser\\%s", file.name);
		FILE* pFile; 
		fopen_s(&pFile, arq, "rt");
...
void ImportItem()
{
	...
	char temp[256];
	char pathError[256];
	strncpy_s(temp, "..\\DATA\\ImportItem\\*.txt", 20);
	...
	while (true)
	{
		...
		sprintf_s(pathSucess, "..\\DATA\\ImportItem\\%s", file.name);
		sprintf_s(pathError, "..\\DATA\\ImportItem\\err\\%s", file.name);
...
void ImportBan()
{
	...
	char temp[256];
	char pathError[256];
	strncpy_s(temp, "..\\DATA\\ImportBan\\*.txt", 20);
	...
	while (true)
	{
		...
		sprintf_s(pathSucess, "..\\DATA\\ImportBan\\%s", file.name);
		sprintf_s(pathError, "..\\DATA\\ImportBan\\err\\%s", file.name);
...
void ImportCash()
{
	...
	char temp[256];
	char pathError[256];
	strncpy_s(temp, "..\\DATA\\ImportCash\\*.txt", 20);
	...
	while (true)
	{
		...
		sprintf_s(pathSucess, "..\\DATA\\ImportCash\\%s", file.name);
		sprintf_s(pathError, "..\\DATA\\ImportCash\\err\\%s", file.name);
...
bool CreateGUI()
{
	...
	const char* szWindowTitle = "W2 - DataBase Server (v761) @SeiTbNao";
