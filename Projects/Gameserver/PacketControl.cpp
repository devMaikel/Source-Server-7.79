...
bool CUser::PacketControl(BYTE *pBuffer, INT32 size)
{
	...
	case 2568:
	{
		...
		if (pUser[clientId].Status == USER_PLAY)
		{
			if (pUser[clientId].LastTimeRequestDrop + 7000 > CurrentTime)
				return 0;
	...
	case 0x399:
	{
		...
		if (Trade.ClientId)
		{
			...
			{
				SendClientMessage(clientId, "N�o � possivel trocar com o modo PvP ativo");
