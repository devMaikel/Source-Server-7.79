...
bool CUser::RequestMerchantNPC(PacketHeader *Header)
{
	...
	INT32 race = pMob[npcId].Mobs.Player.bStatus.Merchant.Merchant,
	      ...
	      level = pMob[npcId].Mobs.Player.bStatus.Level;
...
#pragma region ODIN
		else if (level == 63)
		{
			...
			if (event->Register(*this, nullptr))
				SendSay(npcId, "%s, você foi registrado. Total de registros: %u/52", pMob[clientId].Mobs.Player.Name, event->GetTotalRegistered());
			else
				SendSay(npcId, "Você já está registrado, %s", pMob[clientId].Mobs.Player.Name);
...
#pragma region DRAGaO DE ARZAN
	...
	{ // 004296AD - provavel griupan
		...
		for(LOCAL_655 = 0; LOCAL_655 < 20; LOCAL_655 ++)
		{
...
#pragma region UXMAL
		else if (npcType == 33)
		{
			...
			if ((now.tm_min >= 16 && now.tm_min <= 19) || (now.tm_min >= 36 && now.tm_min <= 39) || (now.tm_min >= 56 && now.tm_min <= 59))
			{
				...
				{
					SendSay(npcId, "O registro da quest pode ser usado apenas pelos lideres do grupo");

					return true;
				}

				INT8 i;
				CMob* lider = &pMob[clientId];
				...
				//if (i < 2) //2
				//{
				//	SendClientMessage(clientId, "Impossivel registrar.");
				...
				if (ref > 6)
				{
					SendSay(npcId, "Não está disponivel.");
