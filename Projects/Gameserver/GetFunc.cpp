...
{
	const STRUCT_CHARINFO& charInfo = mob.Mobs;
	const STRUCT_MOB& player = charInfo.Player;
	int points = 0;
	...
	for (int i = 1; i < 16; i++)
	{
		const STRUCT_ITEM& item = player.Equip[i];
		if (item.Index <= 0 || item.Index >= MAX_ITEMLIST)
			continue;

		int itemType = GetItemAbility(const_cast<STRUCT_ITEM*>(&item), EF_MOBTYPE);
		...
		if (i != 14)
		{
			int sanc = GetItemSanc(const_cast<STRUCT_ITEM*>(&item));
			points += (sanc * 5);
		}
		else
		{
...
void GetCreateMob(int clientId, BYTE *bufPak)
{
	...
	if (clientId < MAX_PLAYER && pUser[clientId].Arena.GroupIndex != -1 && pMob[clientId].Target.X >= 143 && pMob[clientId].Target.Y >= 546 && pMob[clientId].Target.X <= 195 && pMob[clientId].Target.Y <= 625)
	{
		p->GuildIndex = 0;
		p->GuildMemberType = 0;
		...
		sprintf_s(p->pTab, "Grupo %s (%d)", GroupsName[pUser[clientId].Arena.GroupIndex], pUser[clientId].Arena.Kills);
	}
}
...
void GetCreateMobTrade(int clientId, BYTE *bufPak)
{
	...
	strncpy_s(p->StoreName, pMob[clientId].Ingame.AutoTradeName, 24);
}
