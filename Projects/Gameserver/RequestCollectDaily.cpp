...
bool CUser::RequestCollectDaily(PacketHeader* Header)
{
	...
	case 1: // Segunda-feira
	{
		...
		auto diffTime = std::difftime(pMob[clientId].Mobs.DailyQuest.BattlePass.GetTMStruct(), now_time_t);
		...
		auto battlePass = pMob[clientId].Mobs.DailyQuest.BattlePass;
		if (diffTime > 0.0)
		{
		...
		pMob[clientId].Mobs.DailyQuest.BattlePass.Dia = day;
		pMob[clientId].Mobs.DailyQuest.BattlePass.Mes = month;
		pMob[clientId].Mobs.DailyQuest.BattlePass.Ano = year;
		pMob[clientId].Mobs.DailyQuest.BattlePass.Hora = hour;
		pMob[clientId].Mobs.DailyQuest.BattlePass.Minuto = min;
		pMob[clientId].Mobs.DailyQuest.BattlePass.Segundo = sec;
	...
	case 2: // Terça-feira
	{
		...
		pMob[clientId].Mobs.Affects[slot].Time = 1800;
	...
	case 3:
	{
		...
		if (TimeRemaining(pMob[clientId].Mobs.Player.Inventory[60].EFV1, pMob[clientId].Mobs.Player.Inventory[60].EFV2, pMob[clientId].Mobs.Player.Inventory[60].EFV3 + 1900) > 0.0f)
		{
			dia = pMob[clientId].Mobs.Player.Inventory[60].EFV1;
			mes = pMob[clientId].Mobs.Player.Inventory[60].EFV2;
			ano = pMob[clientId].Mobs.Player.Inventory[60].EFV3 + 1900;
		}
		else
		{
			mes = month + 1;
		...
		pMob[clientId].Mobs.Player.Inventory[60].Index = 3467;
		pMob[clientId].Mobs.Player.Inventory[60].EF1 = 106;
		pMob[clientId].Mobs.Player.Inventory[60].EFV1 = dia;
		pMob[clientId].Mobs.Player.Inventory[60].EF3 = 109;
		pMob[clientId].Mobs.Player.Inventory[60].EFV3 = (ano - 1900);
		pMob[clientId].Mobs.Player.Inventory[60].EF2 = 110;
		pMob[clientId].Mobs.Player.Inventory[60].EFV2 = mes;

		STRUCT_ITEM emptyItem{ };
