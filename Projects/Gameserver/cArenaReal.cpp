...
constexpr std::array<STRUCT_POSITION, 4> StartPosition =
{
	{
		{ 190, 621 },
		{ 190, 555 },
...
bool TOD_Arena::Register(CUser& user, STRUCT_ITEM* item)
{
	...
	user.Arena.UsersRegistres += 1;

	SendClientPacket(user.clientId);

	Log(user.clientId, LOG_INGAME, "Registrado na Arena Real");
	Log(SERVER_SIDE, LOG_INGAME, "[%s] - %s registrado na Arena Real. Total: %02d/%02d", user.User.Username, pMob[user.clientId].Mobs.Player.Name, _registeredUsers.size(), 52);
	SendChatMessage(0xFFFF7F27, "%s registrou-se na arena. Total: %02d/%02d", pMob[user.clientId].Mobs.Player.Name, _registeredUsers.size(), 52);
	return true;
}
...
void TOD_Arena::Work()
{
	...
	if (model == -1 && _status == TOD_Arena_Status::Disabled && now.tm_min < 5)
	{
...
bool TOD_Arena::StartArena()
{
	...
	_registeredUsers.erase(std::remove_if(std::begin(_registeredUsers), std::end(_registeredUsers), [&](const CUser* user) {
		return user->Status != USER_PLAY || pMob[user->clientId].Mobs.Player.Equip[0].EFV2 < CELESTIAL;
	...
	std::sort(std::begin(_registeredUsers), std::end(_registeredUsers), [](const CUser* lhs, const CUser* rhs) {
		return GetOverPower(pMob[lhs->clientId]) > GetOverPower(pMob[rhs->clientId]);
		});
	...
	std::stringstream str;
	{
		...
		for (auto& user : _registeredUsers)
		{
			...
			str << user->User.Username << " - " << pMob[user->clientId].Mobs.Player.Name << " entrou no grupo " << GroupsName[groupIndex] << " com " << GetOverPower(pMob[user->clientId]) << " OverPower\n";
	...
	for (auto& group : _groups)
	{
		...
		pMob[groupLeader->clientId].Leader = 0;
		...
		for (auto& memberInfo : group)
		{
			...
			if (user != groupLeader)
				pMob[groupLeader->clientId].PartyList[partyIndex++] = user->clientId;

			SendAddParty(groupLeader->clientId, user->clientId, 0);
			SendAddParty(user->clientId, groupLeader->clientId, 1);

			if (user != groupLeader)
			pMob[user->clientId].Leader = groupLeader->clientId;
			...
			pMob[user->clientId].GuildDisable = 1;
			...
			GridMulticast_2(pMob[user->clientId].Target.X, pMob[user->clientId].Target.Y, reinterpret_cast<BYTE*>(&packet), 0);
		}
...
const int TOD_Arena::IsOnHour()
{
	...
	if (when.tm_hour == 12 || when.tm_hour == 19 || when.tm_hour == 23)
		model = 2;
	else if (when.tm_hour == 11 || when.tm_hour == 18 || when.tm_hour == 22)
		model = 1;
...
void TOD_Arena::Reset()
{
	...
	for (int i = 1; i < MAX_PLAYER; ++i)
	{
		...
		if ((pMob[i].Target.X / 128) == 1 && (pMob[i].Target.Y / 128) == 4)
...
void TOD_Arena::GiveReward(CUser* user, const ArenaReward& reward)
{
	...
	for (const auto item : reward.Items)
	{
		...
		SendItem(user->clientId, SlotType::Inv, slotId, const_cast<STRUCT_ITEM*>(&item));
...
void TOD_Arena::GiveReward(int groupIndex)
{
	...
	{
		SendChatMessage(0xFFFF7F27, "O usuário %s foi o Top Kill com %d kills", pMob[topKill.User->clientId].Mobs.Player.Name, topKill.User->Arena.Kills);
...
bool TOD_Arena::MobKilled(int killed, int killer)
{
	...
	int groupIndex = pUser[killed].Arena.GroupIndex;
	...
	for (int i = 0; i < 32; i++)
	{
		if (pMob[killed].Mobs.Affects[i].Index == 32)
		{
			memset(&pMob[killed].Mobs.Affects[i], 0, sizeof STRUCT_AFFECT);
	...
	pUser[killer].Arena.Kills++;

	pUser[killed].Potion.CountHp = pMob[killed].Mobs.Player.Status.maxHP;
	pMob[killed].Mobs.Player.Status.curHP = pMob[killed].Mobs.Player.Status.maxHP;
	pUser[killed].Potion.CountMp = pMob[killed].Mobs.Player.Status.maxMP;
	pMob[killed].Mobs.Player.Status.curMP = pMob[killed].Mobs.Player.Status.maxMP;

	SetReqHp(killed);
	SendScore(killed);

	Teleportar(killed, StartPosition[pUser[killed].Arena.GroupIndex].X, StartPosition[pUser[killed].Arena.GroupIndex].Y);

	p364 packet{};
	GetCreateMob(killed, (BYTE*)&packet);

	GridMulticast_2(pMob[killed].Target.X, pMob[killed].Target.Y, (BYTE*)&packet, 0);
	SendAffect(killed);

	char message[128] = { 0 };
	sprintf_s(message, ".%s da equipe %s foi morto por %s.", pMob[killed].Mobs.Player.Name, GroupsName[groupIndex], pMob[killer].Mobs.Player.Name);
...
bool TOD_Arena::IsUserValid(const InternalArena& memberInfo) const
{
	...
	if ((pMob[clientId].Target.X / 128) != 1 || (pMob[clientId].Target.Y / 128) != 4)
		return false;
...
void TOD_Arena::RefreshScoreboard() const
{
	...
	for (const auto& group : _groups)
	{
		...
		{
			p691 pak;
			memset(&pak, 0, sizeof p691);

			pak.Header.Size = sizeof p691;
			pak.Header.ClientId = memberInfo.User->clientId;
			pak.Header.PacketId = 0x691;

			for (int a = 0; a < 4; a++)
			{
				strcpy_s(pak.Name[a], GroupsName[a]);
				pak.Point[a] = points[a];
			}

			pUser[memberInfo.User->clientId].AddMessage((BYTE*)&pak, sizeof(p691));
			pUser[memberInfo.User->clientId].SendMessageA();
	...
	for (int i = 1; i < MAX_PLAYER; ++i)
	{
		...
		if ((pMob[i].Target.X / 128) == 1 && (pMob[i].Target.Y / 128) == 4)
		{
			...
			pUser[i].AddMessage((BYTE*)&pak, sizeof(p691));
