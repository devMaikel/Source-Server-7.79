...
void ProcessSecTimer()
{
	...
	if (sServer.isReboot && counter % 2 == 0)
	{
		char msgbot[6][128] =
		{
			"AVISO! Servidor será reiniciado após 1 minuto.",
			"Para salvar seu progresso é recomendável sair do jogo.",
			"Reinicialização em 40 segundos...",
			"Reinicialização em 30 segundos...",
			"Reinicialização em 20 segundos...",
			"Reiniciando..."
		};

		if (ExitRebootCount == 0)
			SendNotice(msgbot[RebootCount]);

		ExitRebootCount++;

		if (!(ExitRebootCount % 10))
		{
			if (RebootCount == 5)
			{
				IsRebootNow = true;
				...
				for (int i = 1; i < MAX_PLAYER; i++)
				{
					...
					if (pUser[i].Status == USER_SELCHAR)
						SaveUser(i, 0);
	...
	{ // Passou um segundo e checa se passou 15 minutos
		...
		for (auto& user : pUser)
		{
			...
			if (mob.Target.X >= 3600 && mob.Target.X <= 3700 && mob.Target.Y >= 3600 && mob.Target.Y <= 3700)
			{
				...
				SendSignalParm(user.clientId, 0x7530, 0x3A1, 60 * 20);
