...
std::vector<LojaDonate> ControlDonateLoja;
...
int GetMobAbility(STRUCT_MOB *usr, int eff)
{
    ...
    int LOCAL_18[20];
    for(int LOCAL_19 = 0; LOCAL_19 < 20; LOCAL_19++)
    {
        LOCAL_18[LOCAL_19] = 0;
		...
		/*if (LOCAL_19 == 12 && eff != EF_SAVEMANA && eff != EF_AC && eff != EF_BONUSEXPIND)
			continue;*/ //trava de adicional de traje
...
bool CanCargo(STRUCT_ITEM *destItem, STRUCT_ITEM *Inventory, int pDestX, int pDestY)
{
	...
	unsigned char invSlots[168];
	memset(invSlots, 0, 168);
...
void MobKilled(int arg1, int arg2, int arg3, int arg4)
{
	...
	if(arg1 < MAX_PLAYER && (pMob[arg1].Mobs.Player.bStatus.Level > 1000))
	{
	...
	{
		int killerId = arg2;
		if (killerId >= MAX_PLAYER && pMob[killerId].Summoner < MAX_PLAYER)
			killerId = pMob[arg2].Summoner;

		if (static_cast<TOD_Arena*>(CEventManager::GetInstance().GetEvent(eEventType::Arena))->MobKilled(arg1, killerId))
			return;
	}
	...
	if(arg2 >= MAX_PLAYER && pMob[arg2].Mobs.Player.CapeInfo == 4 && LOCAL_3 >= 315 && LOCAL_3 <= 345 && arg1 > MAX_PLAYER && pMob[arg1].Mobs.Player.CapeInfo != 4) // sistema de upar montaria
	{
...
int CreateItem(int posX, int posY, STRUCT_ITEM *item, int unknow, int arg4, int status)
{
	...
	if (g_pInitItem[LOCAL_1].Item.Index >= 3145 && g_pInitItem[LOCAL_1].Item.Index <= 3149)
	{
		
		int defaultHeigth = 18;
		if (!UpdateItem(LOCAL_1, 3, &defaultHeigth))
		{
			Log(LOCAL_1, LOG_INGAME, " tower update height error");
			return LOCAL_1;
		}
		...
		LOCAL_150.Header.PacketId = 0x374;
		LOCAL_150.Header.ClientId = 0x7530;
		LOCAL_150.gateId = LOCAL_1 + 10000;
		LOCAL_150.Header.Size = sizeof (p374);
		LOCAL_150.status = g_pInitItem[LOCAL_1].Status;
		LOCAL_150.Height = 18;

		GridMulticast_2(g_pInitItem[LOCAL_1].PosX, g_pInitItem[LOCAL_1].PosY, (BYTE*)&LOCAL_150, 0);
	}
...
{ // ADDRBASE = 0x8B9E778
	...
	INT32 LOCAL_2 = BASE_UpdateItem3(g_pInitItem[arg1].CanRun,
		g_pInitItem[arg1].Status,
		arg2,
		g_pInitItem[arg1].PosX,
		g_pInitItem[arg1].PosY,
		NULL,
		LOCAL_1,
		arg3);
...
int BASE_UpdateItem3(signed int maskidx, int CurrentState, int NextState, int xx, int yy, int a6, int rotate, int* heigth)
{
	//by seitbnao
	...
	if (maskidx < 10 && maskidx >= 0)
	{
		...
		if (acceptMask)
		{
			for (y = 0; y <= 5; ++y)
			{
				for (x = 0; x <= 5; ++x)
				{
					groundMask = g_pGroundMask[maskidx][rotate][y][x];

					if (groundMask)
					{
						groundMask *= acceptMask;
						...
						newMask = g_pHeightGrid[yp - g_HeightPosY][xp - g_HeightPosX] + groundMask;
						...
						g_pHeightGrid[yp - g_HeightPosY][xp - g_HeightPosX] = newMask;
					}
				}
			}
			return 1;
...
void CloseUser(INT32 clientId)
{
	...
	CEventManager::GetInstance().GetEventItem(eEventType::Arena)->Unregister(pUser[clientId]);
	...
	else if (LOCAL_3 == USER_PLAY || LOCAL_3 == USER_SAVING4QUIT)
	{
		...
		memcpy(&packet.Storage, &pUser[clientId].User.Storage.Item, sizeof STRUCT_ITEM * 168);
		...
		memcpy(&packet.Friends, &pUser[clientId].User.Friends, 30 * 16);

		packet.Insignia = pUser[clientId].User.Insignias.Value;

		packet.Slot = -1;

		packet.Unique = pUser[clientId].User.Unique.Value;
		packet.Position.X = pMob[clientId].Target.X;
		packet.Position.Y = pMob[clientId].Target.Y;

		packet.Daily.WeekYear = pUser[clientId].User.Daily.WeekYear;
		...
		packet.SingleGift = pUser[clientId].User.SingleGift;
		memcpy(&packet.Daily.Received[0], pUser[clientId].User.Daily.Received, sizeof(pUser[clientId].User.Daily.Received));
	...
	pUser[clientId].User.Unique.Value = 0;
	...
	pUser[clientId].User.Daily.WeekYear = 0;

	memset(pUser[clientId].User.Daily.Received, 0, sizeof pUser[clientId].User.Daily.Received);
	...
	/*pUser[clientId].Christmas.Mission.MissionId = -1;
	std::fill(std::begin(pUser[clientId].Christmas.Mission.Count), std::end(pUser[clientId].Christmas.Mission.Count), 0);
	pUser[clientId].Christmas.Mission.Status = TOD_ChristmasMission_Status::WaitingNextRound;*/
...
void SaveUser(INT32 clientId, INT32 arg2)
{
	...
	packet.Daily.WeekYear = pUser[clientId].User.Daily.WeekYear;
	memcpy(packet.Daily.Received, pUser[clientId].User.Daily.Received, sizeof pUser[clientId].User.Daily.Received);
...
{
	if (!pMob[clientId].Mobs.DailyQuest.IsAccepted)
	{
		pMob[clientId].Mobs.DailyQuest.IsAccepted = true;
...
{
	STRUCT_ACCOUNT* account = &pUser[clientId].User;
	...
	memcpy(&packet.Item, &sServer.Daily.Item, sizeof STRUCT_ITEM * 7 * 4);
...
void SendArenaScoreboard(int clientId, const std::array<int, 4>& points)
{
	...
	pUser[clientId].AddMessage(reinterpret_cast<BYTE*>(&packet), sizeof MSG_ARENASCOREBOARD);
}
...
std::vector<int> GetMobsInAreaStr(STRUCT_POSITION minPos, STRUCT_POSITION maxPos, int Str)
{
	auto Mobs = std::vector<int>();
	...
	for (int i = 1000; i < MAX_MOB; i++)
	{
		...
		{
			if (Mob->Target.X >= minPos.X && Mob->Target.X <= maxPos.X && Mob->Target.Y >= minPos.Y && Mob->Target.Y <= maxPos.Y && Mob->Mobs.Player.Status.STR == Str)
				Mobs.push_back(static_cast<int>(i));
		}
	}
