...
void SendScore(int clientIndex)
{
	...
	p.Header.Size = sizeof MSG_UpdateScore;

	if (clientIndex < MAX_PLAYER)
	{
		//p.Life = pMob[clientIndex].Lifes;

		p.ReqHp = mob->Status.curHP;
		p.ReqMp = mob->Status.curMP;
	}

	p.Guild = (mob->GuildIndex);
	p.GuildLevel = mob->GuildMemberType;

	if (pMob[clientIndex].GuildDisable != 0)
		p.Guild = 0;

	p.Magician = static_cast<unsigned short>(pMob[clientIndex].MagicIncrement);
	p.Rsv = pMob[clientIndex].Mobs.AffectInfo;

	p.SaveMana = mob->RegenMP;

	p.Critical = mob->Critical;

	memcpy(&p.Resist[0], &mob->Resistence[0], 0x4);


	memcpy(&p.Score, &mob->Status, sizeof STRUCT_STATUS);
	...
	for (int i = 0; i < 32; i++)
	{
		...
		int time = affect[i].Time;

		if (time > 255)
			time = 255;

		p.Affect[i] = (index << 0x8) + (time & 0xFF);
	 }
...
{
	if(!pMob[tradeClientId].Ingame.IsAutoTrading)
		return;
	...
	memcpy(pTrade.Gold, pMob[tradeClientId].Ingame.AutoTrade.Price, sizeof(int) * 12);
	memcpy(pTrade.Item, pMob[tradeClientId].Ingame.AutoTrade.Item, 12 * sizeof(STRUCT_ITEM));
	memcpy(pTrade.Slot, pMob[tradeClientId].Ingame.AutoTrade.Slots, 8);
	strncpy_s(pTrade.Name, pMob[tradeClientId].Ingame.AutoTrade.Name, 24);
	pTrade.Unknown = pMob[tradeClientId].Ingame.AutoTrade.Unknown_1784;
...
void SendEtc(int clientId)
{
	...
	p.LearnedSkill[0] = mob->Learn[0];
	p.LearnedSkill[1] = mob->Learn[1];
 

	p.SpecialBonus = mob->MasterPoint;
	p.SkillBonus = mob->SkillPoint;
	p.ScoreBonus = mob->StatusPoint;
...
void SendMissionInfo(int clientId)
{
	...
	const auto& daily = pMob[clientId].Mobs.DailyQuest;
	if (daily.QuestId == -1)
	{
		pUser[clientId].AddMessage(reinterpret_cast<BYTE*>(&packet), sizeof packet);
	...
	int evolution = pMob[clientId].Mobs.Player.Equip[0].Effect[1].Value;
	int level = pMob[clientId].Mobs.Player.Status.Level;

	if ((evolution <= ARCH && level == MAX_LEVEL) || (evolution >= CELESTIAL && level >= MAX_LEVEL_CELESTIAL))
		exp = 0;

	if (pMob[clientId].Mobs.Player.Equip[0].Effect[1].Value != MORTAL)
