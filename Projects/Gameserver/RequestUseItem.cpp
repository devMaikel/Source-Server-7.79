...
#pragma region amago
		else if (_volatile == 16)
		{
			...
			STRUCT_ITEM* dstItem = GetItemPointer(clientId, (short)SlotType::Equip, 14);
			if (dstItem->Index < 2330 || dstItem->Index > 2389)
			{
				SendClientMessage(clientId, g_pLanguageString[_NN_Mount_Not_Match]);
			...
			int amount = GetItemAmount(srcItem);
			/*if (amount != 1 || amount < 0 || amount > 120)
			{
				SendClientMessage(clientId, "Necessario 120 unidades de Pa de Brinco");

				SendItem(clientId, SlotType::Inv, p->SrcSlot, srcItem);
				Log(SERVER_SIDE, LOG_INGAME, "Valor para uso de âmagos incorreto. Total de amount: %d. Valor enviado: %d", amount, amount);
				return true;
			}*/
			...
			Log(clientId, LOG_INGAME, "Solicitado o uso de %d âmagos", amount);
			...
			for (int i = 0; i < amount; ++i)
			{
				...
				else
				{
					...
					{
						SendClientMessage(clientId, "Level maximo.");
						break;
					}
