...
void TOD_Mission::Reset(CUser& user) const
{
	...
	auto& daily = pMob[user.clientId].Mobs.DailyQuest;
...
void TOD_Mission::MobKilled(int killer, int killed) const
{
	...
	// Checa se é evocação
	if (killer >= MAX_PLAYER && pMob[killer].Mobs.Player.CapeInfo != 4)
		return;

	if (killer >= MAX_PLAYER && pMob[killer].Mobs.Player.CapeInfo == 4)
		killer = pMob[killer].Summoner;

	if (killer >= MAX_PLAYER)
		return;

	std::string mobName{ pMob[killed].Mobs.Player.Name };

	INT32 leaderId = pMob[killer].Leader;
	if (leaderId == 0)
		leaderId = killer;
	...
	for (size_t i = 0; i < 5; i++)
	{
		for (int iParty = 0; iParty < 13; iParty++)
		{
			...
			if (iParty != 12)
				memberId = pMob[leaderId].PartyList[iParty];

			if (memberId <= 0 || memberId >= MAX_PLAYER)
				continue;

			auto& daily = pMob[memberId].Mobs.DailyQuest;;
			if (daily.QuestId == -1 || !daily.IsAccepted)
				continue;

			if (memberId != killer)
			{
				INT32 distance = GetDistance(pMob[memberId].Target.X, pMob[memberId].Target.Y, pMob[killer].Target.X, pMob[killer].Target.Y);
			...
			Log(memberId, LOG_INGAME, "%s matou o mob %s da quest %s (%d). Total: %03d/%03d", pMob[killer].Mobs.Player.Name, mobName.c_str(), questIt->QuestName.c_str(), questIt->QuestId, daily.MobCount[i], quest.Mob.Amount[i]);
